...
features["btc_rel_vol_20"] = (
    btc["volume"] / btc["volume"].rolling(20, min_periods=20).mean()
)
...
features["corr_btc_spx_60"] = (
    features["btc_ret_1d"].rolling(60, min_periods=60).corr(spx_td["spx_ret_1d"])
)

# ── Fear & Greed Index ──
# FNG доступен только с 2018-02-01. Чтобы не отрезать 5 лет истории BTC (2013-2017),
# применяем missing-indicator подход: считаем индикатор пропуска и импутируем
# нейтральным значением 50 (граница Fear/Greed).
fng_value_raw = fng["value"]
features["fng_missing"] = fng_value_raw.isna().astype(int)
features["fng_value"] = fng_value_raw.fillna(50.0)
features["fng_ma7"] = fng_value_raw.rolling(7, min_periods=7).mean().fillna(50.0)
features["fng_change_7d"] = fng_value_raw.diff(7).fillna(0.0)
