...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('src/etl/coindesk.log'),
        logging.StreamHandler(sys.stdout)
    ]
)


async def fetch_url(session, url, params={}, headers={}, retries_left=RETRIES):
    async with semaphore:
        try:
            async with session.get(url, params=params, headers=headers) as response:
                if response.status == 429 and retries_left:
                    await asyncio.sleep(RETRY_BACKOFF)
                    return await fetch_url(session, semaphore, url, params, headers, retries_left=retries_left-1)
...
async def fetch_daily_ohlcv(start_ts, end_ts):
    ...
    async with aiohttp.ClientSession() as session:
        for from_ts in range (start_ts, end_ts + step*limit, step * limit):
            to_ts = min(from_ts + limit*step, end_ts)
            ...
            task = asyncio.create_task(fetch_url(session, url, params))
            tasks.append(task)
